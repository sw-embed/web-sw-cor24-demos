...
The Status tab (`/#/status`) shows live data that must be periodically refreshed.
The generation tools live under `tools/` and are **NOT** part of the Cargo workspace.
They must be run with `--manifest-path`.

### Quick refresh (issue counts + issue progress charts)

```bash
cargo run --manifest-path tools/gen-status/Cargo.toml      # open issue counts + agentrail status
cargo run --manifest-path tools/gen-issue-chart/Cargo.toml  # issue progress SVG charts
./scripts/build-pages.sh                                    # build WASM, copy to pages/
git add src/data/status/generated.rs images/*-issue-chart.svg pages/
...
| Tool | Fetches | Output | Coverage |
|------|---------|--------|----------|
| `tools/gen-status` | Open issue counts + agentrail plan/steps via `gh api` | `src/data/status/generated.rs` | 34 repos |
| `tools/gen-issue-chart` | All issues (open+closed) with dates via `gh api` | `images/{slug}-issue-chart.svg` | 12 repos |

### Activity reports (commits heatmap, closed-issues timeline)

The `reports/commits.html` and `reports/closed-issues.html` files are generated by
scripts in the **`sw-cor24-project`** repo, not this repo:

```bash
cd ~/github/sw-embed/sw-cor24-project
./scripts/gen-commits.sh          # all commits across repos
./scripts/gen-closed-issues.sh    # all closed issues across repos
```

Copy the generated HTML into this repo's `reports/` directory, then rebuild.

### Time zones

- `gen-status` fetches counts only (no timestamps).
- `gen-issue-chart` converts GitHub UTC timestamps to local time via `chrono::Local`.
- Activity report scripts convert UTC to local time via `date` (macOS) / `date -d` (Linux).
